...
export default function PatientDetail() {
  ...
  useEffect(() => {
    ...
    const labels = patient.visits.map((v, i) => `Visit ${i + 1} (${v.ga}w)`)
    const sbpData = patient.visits.map(v => v.sbp)
    const dbpData = patient.visits.map(v => v.dbp)
    ...
    <div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 9px',
          ...
          {patient.risk === 'normal' ? 'Normal' : patient.risk === 'high' ? 'High risk' : 'Medium'}
        </span>
      </div>
      ...
      <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
        ...
        {alertResult && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button onClick={() => toast.success(`Alert acknowledged for ${patient.name}`)}
                style={{ background: '#dcfce7', color: '#166534', border: 'none',
                  borderRadius: 7, padding: '6px 14px', fontSize: 12,
                  fontWeight: 600, cursor: 'pointer' }}>
                Mark acknowledged
              </button>
              <button onClick={() => toast.error(`${patient.name} flagged for specialist review`)}
            ...
            {[
              ...
              return (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between',
                  padding: '5px 0', borderBottom: '1px solid #f4f4f4', fontSize: 13 }}>
                  <span style={{ color: '#9ca3af' }}>{k}</span>
                  <span style={{ fontWeight: 600, color: isDanger ? '#dc2626' : isWarn ? '#d97706' : '#111' }}>
                    {v}
                  </span>
                </div>
              )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {patient.visits.map((v, i) => {
            ...
            const protColor = v.proteinuria === 'None' ? ['#dcfce7','#166534'] : v.proteinuria === 'Trace' ? ['#fef3c7','#92400e'] : ['#fee2e2','#991b1b']
            return (
              <div key={i} style={{
                display: 'grid',
